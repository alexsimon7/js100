...
/*notes: The above solution was an attempt to create 'if' statements to removes other potential values based on the fact that NaN is falsy. Removing
         ...
         those values, only a few were of the number type (0, -0, 0n, and NaN). After removing values that were not the number type, 0 -0 (which equal)
         and 0n left NaN as the only option by elimination.
